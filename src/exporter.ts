import {
  AudioBufferSource,
  BufferTarget,
  CanvasSource,
  Mp4OutputFormat,
  Output,
  StreamTarget,
} from "mediabunny";
import { streamReplayAudio } from "./audio";
import {
  createCommitGatedStream,
  estimateMp4SizeBytes,
  EXPORT_STREAM_CHUNK_BYTES,
  formatExportSize,
  MEMORY_EXPORT_LIMIT_BYTES,
  type PreparedExportDestination,
} from "./export-destination";
import { AssetMediaStore, ChatCanvasRenderer } from "./renderer";
import type { CompiledTimeline, ExportPreset, ImportedProject, RenderTheme } from "./types";

export interface ExportProgress {
  phase: "prepare" | "render" | "finalize";
  progress: number;
  frame: number;
  totalFrames: number;
}

export interface ExportResult {
  blob: Blob;
  cleanup: () => Promise<void>;
}

export interface ExportOptions {
  project: ImportedProject;
  timeline: CompiledTimeline;
  preset: ExportPreset;
  theme: RenderTheme;
  mediaStore: AssetMediaStore;
  incomingSound: boolean;
  destination: PreparedExportDestination;
  fps?: number;
  signal?: AbortSignal;
  onProgress?: (progress: ExportProgress) => void;
}

function abortError(): DOMException {
  return new DOMException("Der Export wurde abgebrochen.", "AbortError");
}

interface OutputTargetSession {
  target: BufferTarget | StreamTarget;
  armCommit(): void;
  result(): Promise<ExportResult>;
  discard(reason?: unknown): Promise<void>;
}

async function createOutputTargetSession(destination: PreparedExportDestination): Promise<OutputTargetSession> {
  if (destination.kind === "memory") {
    const target = new BufferTarget();
    return {
      target,
      armCommit: () => undefined,
      result: async () => {
        if (!target.buffer) throw new Error("Der Video-Encoder hat keine Datei erzeugt.");
        return {
          blob: new Blob([target.buffer], { type: "video/mp4" }),
          cleanup: async () => undefined,
        };
      },
      discard: async () => undefined,
    };
  }

  const writer = await destination.handle.createWritable({ keepExistingData: false });
  const gated = createCommitGatedStream(writer);
  const target = new StreamTarget(gated.stream, {
    chunked: true,
    chunkSize: EXPORT_STREAM_CHUNK_BYTES,
  });
  const cleanup = destination.kind === "opfs" ? destination.cleanup : async () => undefined;
  return {
    target,
    armCommit: gated.armCommit,
    result: async () => {
      const blob = await destination.handle.getFile();
      return { blob, cleanup };
    },
    discard: async (reason) => {
      await gated.discard(reason).catch(() => undefined);
      await cleanup();
    },
  };
}

export async function exportReplayMp4(options: ExportOptions): Promise<ExportResult> {
  const fps = options.fps ?? 30;
  if (!("VideoEncoder" in window)) {
    throw new Error("Dieser Browser unterstützt keinen schnellen MP4-Export. Bitte Chrome, Edge oder Safari in einer aktuellen Version verwenden.");
  }
  if (options.destination.kind === "memory") {
    const estimatedBytes = estimateMp4SizeBytes(options.timeline.duration, options.preset.bitrate);
    if (estimatedBytes > MEMORY_EXPORT_LIMIT_BYTES) {
      throw new Error(
        `Das MP4 wird voraussichtlich etwa ${formatExportSize(estimatedBytes)} groß. `
        + "Der In-Memory-Export wurde zum Schutz vor einem Absturz blockiert. Bitte verwende einen Browser, "
        + "der direkt in eine Datei schreiben kann, oder verkleinere Zeitraum beziehungsweise Auflösung.",
      );
    }
  }
  options.onProgress?.({ phase: "prepare", progress: 0, frame: 0, totalFrames: 0 });
  await options.mediaStore.preloadForMessages(
    options.timeline.events.map((event) => event.message),
    (done, total) => options.onProgress?.({ phase: "prepare", progress: total ? done / total : 1, frame: done, totalFrames: total }),
    options.signal,
  );
  if (options.signal?.aborted) throw abortError();
  const scheduledAudioAssets = options.mediaStore.getScheduledAudioAssets(options.timeline);
  const needsAudioTrack = options.incomingSound || scheduledAudioAssets.length > 0;
  if (needsAudioTrack && !("AudioEncoder" in window)) {
    throw new Error("Dieser Browser kann den Replay-Ton nicht in MP4 encodieren. Bitte einen aktuellen Chrome-, Edge- oder Safari-Browser verwenden.");
  }

  const canvas = document.createElement("canvas");
  canvas.width = options.preset.width;
  canvas.height = options.preset.height;
  const renderer = new ChatCanvasRenderer(canvas, options.mediaStore);
  renderer.setParticipants(options.project.chat.participants);
  const targetSession = await createOutputTargetSession(options.destination);
  let output: Output;
  let source: CanvasSource;
  let audioSource: AudioBufferSource | null;
  try {
    output = new Output({ format: new Mp4OutputFormat({ fastStart: false }), target: targetSession.target });
    source = new CanvasSource(canvas, {
      codec: "avc",
      bitrate: options.preset.bitrate,
      keyFrameInterval: 2,
      latencyMode: "quality",
    });
    output.addVideoTrack(source);
    audioSource = needsAudioTrack
      ? new AudioBufferSource({
        codec: "aac",
        bitrate: 128_000,
        transform: { sampleRate: 48_000, numberOfChannels: 2 },
      })
      : null;
    if (audioSource) output.addAudioTrack(audioSource);
  } catch (error) {
    await targetSession.discard(error);
    throw error;
  }
  const frameDuration = 1 / fps;
  const totalFrames = Math.max(1, Math.ceil(options.timeline.duration * fps));
  let audioPromise: Promise<void> = Promise.resolve();
  const audioFailure: { error?: unknown } = {};
  let videoClosed = false;
  let exportSessionActive = false;
  const closeVideo = (): void => {
    if (!videoClosed) {
      source.close();
      videoClosed = true;
    }
  };
  try {
    await output.start();
    await options.mediaStore.beginExportSession(options.timeline, fps);
    exportSessionActive = true;
    const rawAudioPromise = audioSource
      ? streamReplayAudio(
        audioSource,
        options.timeline,
        options.theme.selfName,
        scheduledAudioAssets,
        (path, start, duration) => options.mediaStore.loadAudioSegment(path, start, duration),
        options.incomingSound,
        options.signal,
      )
      : Promise.resolve();
    audioPromise = rawAudioPromise.catch((error: unknown) => {
      audioFailure.error = error;
    });
    for (let frame = 0; frame < totalFrames; frame += 1) {
      if (options.signal?.aborted) throw abortError();
      if ("error" in audioFailure) throw audioFailure.error;
      const time = frame * frameDuration;
      await renderer.prepareFrame(options.timeline, time, true);
      renderer.render(options.timeline, time, options.theme);
      await source.add(time, frameDuration, { keyFrame: frame % (fps * 2) === 0 });
      if (frame % 5 === 0 || frame === totalFrames - 1) {
        options.onProgress?.({ phase: "render", progress: (frame + 1) / totalFrames, frame: frame + 1, totalFrames });
      }
      if (frame % 30 === 0) await new Promise<void>((resolve) => requestAnimationFrame(() => resolve()));
    }
    closeVideo();
    await audioPromise;
    if ("error" in audioFailure) throw audioFailure.error;
    await options.mediaStore.endExportSession();
    exportSessionActive = false;
    options.onProgress?.({ phase: "finalize", progress: 0.98, frame: totalFrames, totalFrames });
    targetSession.armCommit();
    await output.finalize();
  } catch (error) {
    closeVideo();
    try { audioSource?.close(); } catch { /* It may already be closed by the streaming task. */ }
    await Promise.allSettled([audioPromise, exportSessionActive ? options.mediaStore.endExportSession() : Promise.resolve()]);
    if (output.state !== "canceled" && output.state !== "finalized") await output.cancel().catch(() => undefined);
    await targetSession.discard(error);
    throw error;
  }
  options.onProgress?.({ phase: "finalize", progress: 1, frame: totalFrames, totalFrames });
  try {
    return await targetSession.result();
  } catch (error) {
    await targetSession.discard(error);
    throw error;
  }
}
